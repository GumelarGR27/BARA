...
<body>
  <nav id="nav" class="navbar navbar-full">
      ...
      <a href="<?php echo base_url('/tabel_invoice') ?>"><button type="button" class="btn btn-info" style="margin-bottom: 20px; float: left; margin-left: 10%;"  type="button">Buat Invoice Baru</button></a>
...
<table class="table table-striped" style = "width:80%; margin-bottom: 100px;">
  ...
  <tr>
    ...
    <th class="table-bordered">Kode Invoice</th>
  ...
  <tr>
    ...
    <td class="table-bordered"><?= $row['tanggal_invoice']; ?></td>
    <td class="table-bordered"><?= $row['nama_perusahaan'];

        // $db = \Config\Database::connect();
        // $query   = $db->query('SELECT * FROM tagihan WHERE nama_perusahaan="'.$row['nama_perusahaan'].'"');
        // $results = $query->getResult(); 
        // foreach ($results as $roww) {
        //         echo $roww->nama_perusahaan;
        //      }
      ?></td>
    <td class="table-bordered"><?= $row['no_invoice']; ?></td>
    <td class="table-bordered">
      <a href="<?php echo base_url('datainv?noinv='.$row['no_invoice']); ?>" data-toggle="tooltip" style="text-decoration: none;" title="View"> 
      <button type="button" class="btn btn-info">View</button></a>
      <a href="<?php echo base_url('Edit_Invoice?noinv='.$row['no_invoice']); ?>"><button type="button" class="btn btn-warning">Edit</button></a>
      <button type="button" class="btn btn-info btn-danger delete" data-id="<?= $row['id_tagihan']; ?>" data-toggle="modal" data-target="#myModal">Delete</button>
    </td>
...
<form action="<?php echo base_url('hapusinvoice'); ?>" method="post">
